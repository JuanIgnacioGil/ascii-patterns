...
The bugs are competing in a race. There two type of bugs. Those facing east (bug_east.txt) and those facing west(bug_west.txt). All bugs run at the same pace. The team (east, west) whose bugs reach the limits (position 90) of the text file (landscape.txt) first wins.
